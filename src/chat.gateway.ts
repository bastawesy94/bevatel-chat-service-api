...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleJoinRoom(client: Socket, roomName: string) {
      ...
      const messages = await this.messageRepository.findMessagesByRoom(room.id, 20, 0);
      client.emit('messageHistory', messages);
      this.server.to(roomName).emit('userJoined', { roomName });
    }
  
    @SubscribeMessage('sendMessage')
    async handleSendMessage(client: Socket, { roomName, content, userId }: { roomName: string; content: string; userId: number }) {
      const room = await this.roomRepository.findByName(roomName);
      if (!room) {
        client.emit('error', `Room with name ${roomName} not found`);
        return;
      ...
      const message = await this.messageRepository.create(content, room.id, userId);
